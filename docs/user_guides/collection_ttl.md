...
# Manipulation
We provide a property of collection level to config TTL: `ttl_seconds`. 
There is a priority for the collection TTL configuration, attempting to get the `ttl_seconds` 
...
```python
collection = Collection(name=name,vschema=schema,properties={"ttl_seconds": 15})
```
## Set TTL for an old collection
The following example is modifying TTL to 1800 seconds:
```python
collection.set_properties(properties={"ttl_seconds": 1800})
```
